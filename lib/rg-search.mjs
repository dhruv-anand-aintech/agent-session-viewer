 ...
 * Query is treated as a regex; invalid regex falls back to fixed-string.
 *
 * @param {string} query   — regex pattern, case-insensitive
...
export async function rgGlobalSearch(query, opts = {}) {
  ...
  // Try regex first; if rg exits with "regex parse error" fall back to literal
  const args = [...baseArgs, query, ...dirs.map(d => d.dir)]
  let raw = await runRg(args)
  if (raw === null) {
    // rg error (likely invalid regex) — retry as fixed string
    raw = await runRg([...baseArgs, "--fixed-strings", query, ...dirs.map(d => d.dir)])
  }
...
function runRg(args) {
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", code => {
      // rg exit codes: 0 = matches found, 1 = no matches, 2 = error (e.g. bad regex)
