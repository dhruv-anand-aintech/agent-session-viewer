...
export async function indexSessionMessages(projectPath, sessionId, msgs, flattenFn) {
  ...
  try {
    ...
    if (!idxs.some(ix => ix.columns?.includes("text"))) {
      ...
      console.log(`[perf lancedb] FTS createIndex: ${(performance.now()-_tFtsStart).toFixed(0)}ms`)
    }
  } catch { /* listIndices not available in all versions */ }

  const _tTotal = performance.now()
  if (_tTotal - _t0 > 50) {
    console.log(`[perf lancedb] indexSessionMessages ${sessionId.slice(0,8)} rows:${records.length} chunk:${(_tChunk-_t0).toFixed(0)}ms open:${(_tOpen-_tChunk).toFixed(0)}ms delete:${(_tDelete-_tOpen).toFixed(0)}ms add:${(_tAdd-_tDelete).toFixed(0)}ms total:${(_tTotal-_t0).toFixed(0)}ms`)
  }
...
export async function getIndexedKeys() {
  ...
  try {
    ...
    console.log(`[perf lancedb] getIndexedKeys: ${(performance.now()-_t0).toFixed(0)}ms — ${rows.length} rows → ${result.size} sessions`)
    return result
