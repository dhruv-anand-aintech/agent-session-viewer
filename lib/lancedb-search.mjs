 ...
 */
export async function searchSessions(query, limit = 60, { skipVector = false } = {}) {
  ...
  // Vector (only when semantic enabled + model ready + not suppressed)
  if (_embedderReady && !skipVector) {
 ...
 */
export async function searchThread(projectPath, sessionId, query, limit = 40, { skipVector = false } = {}) {
  const table = await getTable()
  if (!table) return null
