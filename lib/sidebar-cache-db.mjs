...
// Claude creates placeholder JSONL files while a process is starting. They are
// not sessions until they contain at least one record; keeping them in the
// persistent sidebar cache produces misleading empty rows (and stale rows
// survive restarts). Other providers may legitimately report zero cheaply
// while their transcript is being read, so this rule is intentionally Claude-only.
export function isVisibleSidebarEntry(entry) {
  return !(entry?.source === "claude" && Number(entry.messageCount ?? 0) <= 0)
}
...
function migrateJsonIfNeeded(configDir) {
  ...
  const tx = _db.transaction(rows => {
    for (const e of rows) if (isVisibleSidebarEntry(e)) insert.run(entryToRow(e))
  })
  tx(sessions)
...
export function openSidebarCacheDb(configDir) {
  ...
  _db.exec(SCHEMA)
  // One-time cleanup for caches written by older versions.
  _db.prepare("DELETE FROM sessions WHERE source = 'claude' AND message_count <= 0").run()
...
/** Delete one cached session. Returns the deleted entry, or null when absent. */
export function deleteSidebarEntry(sessionId) {
...
export function upsertSidebarEntry(partial) {
  ...
  if (!isVisibleSidebarEntry(partial)) {
    deleteSidebarEntry(sessionId)
