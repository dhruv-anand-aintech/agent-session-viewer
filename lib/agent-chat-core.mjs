...
export const AGL_MODEL_OPTIONS_BY_AGENT = {
  ...
  codex: [
    { value: "gpt-5.6", label: "GPT-5.6 (auto / Sol)", model: "gpt-5.6", modelClass: "pro" },
    { value: "gpt-5.6-sol", label: "GPT-5.6 Sol", model: "gpt-5.6-sol", modelClass: "pro" },
    { value: "gpt-5.6-terra", label: "GPT-5.6 Terra", model: "gpt-5.6-terra", modelClass: "pro" },
...
let codexAuthCacheAt = 0

function getCodexAuthStatus() {
  const now = Date.now()
  if (codexAuthCache && now - codexAuthCacheAt < 60_000) return codexAuthCache
  const codexPath = commandPath("codex")
  if (!codexPath) {
    codexAuthCache = { kind: "unknown", authenticated: false, label: "Codex CLI unavailable" }
  } else {
    ...
    codexAuthCache = /logged in using chatgpt/i.test(output)
      ? { kind: "chatgpt", authenticated: true, label: "ChatGPT subscription" }
      : /logged in using (?:an )?api key|openai_api_key/i.test(output)
        ? { kind: "api-key", authenticated: true, label: "OpenAI API key" }
        : { kind: "unknown", authenticated: result.status === 0, label: result.status === 0 ? "Codex CLI authenticated" : "Codex CLI login required" }
  }
  codexAuthCacheAt = now
...
export function summarizeRecentChats(chats, maxChars = 12000) {
  const sections = []
  let chars = 0
  for (const chat of Array.isArray(chats) ? chats : []) {
    const transcript = summarizeTranscript(chat?.messages ?? [], 12, 5000)
    if (!transcript) continue
    const heading = [
      `Chat: ${chat.title ?? chat.sessionId ?? "Untitled"}`,
      `Source: ${chat.source ?? "unknown"}`,
      `Project: ${chat.projectPath ?? "unknown"}`,
      `Session ID: ${chat.sessionId ?? "unknown"}`,
      chat.lastActivity ? `Last activity: ${chat.lastActivity}` : null,
    ].filter(Boolean).join("\n")
    const section = `${heading}\n\n${transcript}`
    sections.push(section)
    chars += section.length
    while (chars > maxChars && sections.length > 1) {
      chars -= sections.shift()?.length ?? 0
    }
  }
...
export function buildAgentPrompt({ userPrompt, prompt, conversation, sessionContext }) {
  ...
  const recentChats = summarizeRecentChats(sessionContext?.recentChats ?? [])
  const transcriptLocations = Array.isArray(sessionContext?.transcriptLocations)
    ? sessionContext.transcriptLocations.filter(location => typeof location === "string" && location.trim())
    : []
  const allTranscripts = sessionContext?.transcriptScope === "all"
  const sessionLines = [
  ...
  return [
    "You are a full Codex coding session helping continue work from an existing agent transcript.",
    "Use shell and code tools when they provide current evidence, and answer the user's latest message directly.",
    "Do not expose implementation details about agl unless the user asks.",
    "You may inspect the workspace, source code, git state, logs, running processes, ports, and current commands.",
    "For current process, command, job, or service questions, inspect live machine state instead of relying only on supplied transcript context.",
    allTranscripts ? "You are in transcript research mode. You may read and search transcript files and databases with shell and database tools, but never modify, move, or delete transcript sources." : null,
    allTranscripts ? "Other workspace changes are allowed only when the user's latest request asks you to make a change." : null,
    allTranscripts ? "When evidence spans chats, cite the source agent, project or session title, and distinguish confirmed completion from a plan or unresolved request." : null,
    "",
    "SESSION",
    sessionLines,
    "",
    transcript ? `RECENT TRANSCRIPT\n${transcript}` : "RECENT TRANSCRIPT\nNo transcript messages were supplied.",
    recentChats ? `RECENT CHATS ACROSS AGENTS\n${recentChats}` : null,
    transcriptLocations.length ? `READ-ONLY TRANSCRIPT LOCATIONS\n${transcriptLocations.map(location => `- ${location}`).join("\n")}` : null,
    "",
    chat ? `CHAT SO FAR\n${chat}` : null,
    "",
    "USER MESSAGE",
