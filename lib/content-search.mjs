...
async function searchCodexDir(dir, re, limit, results) {
  ...
  function walk(d) {
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      const projectPath = d
      const key = `codex:${projectPath}:${sessionId}`
      if (results.size < limit) {
        // Can't await inside sync walk; collect paths instead
        _codexPending.push({ filePath, sessionId, projectPath, key })
      }
    }
  }
  const _codexPending = []
  walk(dir)
  for (const { filePath, sessionId, projectPath, key } of _codexPending) {
...
export async function contentSearch(query, { limit = 100 } = {}) {
  ...
  /** @type {Map<string, {source:string, projectPath:string, sessionId:string, snippets:string[]}>} */
