...
export async function startBackgroundIndexer(getRowsFn, readMsgsFn) {
  ...
  // Warm (and if needed download) the embedding model first
  await warmEmbedder()

  if (!isEmbedderReady()) {
    console.warn("[lancedb-indexer] Embedding model unavailable — background indexing skipped")
    _running = false
    return
  }
  ...
  try {
    ...
    if (!todo.length) {
      console.log(`[lancedb-indexer] Index up to date (${indexed.size} sessions)`)
      return
    }

    const total = todo.length
    console.log(`[lancedb-indexer] Indexing ${total} new sessions (${indexed.size} already indexed)`)
    ...
    for (const row of todo) {
      ...
      process.stdout.write(`\r[lancedb-indexer] ${bar(done, total)}`)
      // Yield between sessions so HTTP requests aren't starved
      await new Promise(r => setImmediate(r))
    }

    process.stdout.write("\n")
    console.log(`[lancedb-indexer] Done. Total indexed: ${_indexedCount} sessions`)
  } catch (err) {
    _lastError = err.message
    console.error("[lancedb-indexer] Fatal error:", err.message)
  } finally {
    _running = false
  }
}
